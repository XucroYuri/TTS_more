...
- `data/templates/characters.example.json`：角色库示例。
- `data/templates/demo-hollywood/project.json`：脱敏的标准剧本演示项目。
- `docs/`：设计、计划、发布治理和演示剧本生成 prompt。
...
4. `feat: 新增脱敏标准剧本演示项目`
   - Hollywood screenplay 模板、演示 prompt、空状态/演示状态验证。
5. 推送私有远端并创建中文 PR。
...
可提交文件中不得出现本机绝对路径、局域网地址、UNC 路径、真实角色训练名或真实音频路径。
