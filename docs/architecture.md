...
后端 `requires-python = ">=3.10,<3.12"`，无 OS 标记。`supervisor.py` 用 `_is_windows()` 区分进程组/信号分支。启动脚本同时提供 `scripts/start-dev.ps1`（Windows）和 `scripts/start-dev.sh`（POSIX），`Makefile` 跨平台。CI 在 ubuntu + windows 矩阵上跑。
